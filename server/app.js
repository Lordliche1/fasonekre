...
const universalAuthRouter = require('./routes/universal-auth')
...
app.use('/api/v1/gemini', authenticateUser, geminiRouter)
app.use('/api/v1/serviceman', authenticateUser, roleAuthenticationMiddleware('serviceman'), servicemanRouter)
app.use('/api/v1/department', authenticateOfficer, roleAuthenticationMiddleware('officer'), departmentRouter)
...
const start = async () => {
  try {
    ...
    app.listen(port, () => {
      ...
      console.log(`🔄 Serveur redémarré : ${new Date().toLocaleTimeString()}`);
    });
  } catch (error) {
    console.log(error);
  }
